...
class AG:
    def __init__(self, mutacao, crossover, tamPopulacao, fitnes, cidades, maxdistancia, geracoes, numthreads,
                 naoalcanco):
        start = time()
    ...
    def __nextGeracao(self):
        try:
            threads = []
            tamMutacao = round(
                ceil(float(self.tamPopulacao) / 2) * self.mutacao)  # NUMERO DE INDIVIDUOS QUE SOFRERAO MUTACAO
            faixa = int(ceil(tamMutacao / self.NUM_THREADS))
            r = int((
                                tamMutacao > self.NUM_THREADS and self.NUM_THREADS or tamMutacao))  # VERIFICA SE NUM THREADS E MAIOR QUE NUM ELEMENTOS QUE SOFRERAO MUTACAO
            for i in range(r):
                threads.append(self.__startThreads('self.geraMutacao', {'ini': i + 1,
        ...
        try:
            ...
            while ini <= limite:
                ...
                if flag:
                    ...
                    del self.populacao[ini][
                        posCorte:]  # DELETO O RESTO DO VETOR A PARTIR DO PONTO DE CORTE PARA INSERIR NOVOS ELEMENTOS
                    sorteados = []
                    i = 0
                    j = 0
                while tenhoCidades:
            ...
            for j in cidades:
                if j not in self.distancia[i]:  # SE J NAO ESTA NO DICIONARIO DA CIDADE I VOU INCLUI-LA
                    ...
                    if i == j:
                        ...
                        continue
                    alcanco = random.randint(1,
                                             self.NAOALCANCO)  # CHANCE DE CIDADES NAO SE ALCANCAREM = 1 EM NAO ALCANCO (PARAMETRO)
                    if alcanco == 1:
                        self.distancia[i][j] = 100000  # ATRIBUI VALOR DISTANCIA MUITO ALTO = CIDADE NAO ALCANCAVEL
                        continue
                    self.distancia[i][j] = random.randint(1,
                                                          self.MAX_DIST)  # SORTEIA VALOR DA DISTANCIA ALEATORIAMENTE ENTRE 1 - MAX_DIST SE FOR A MESMA CIDADE VALOR DISTANCIA 0
        return
    ...
    def geraPopulacao(self):
        ...
        try:
            for i in range(tam):
                ...
                cont = 1
                while not self.__sublist(d,
                                         self.cidades):  # RODA LOOP ENQUANTO NAO TEM TODAS CIDADES NA ROTA - SE IMPASSE ATE 100 ITERACOES - COMECA DE NOVO
                    ...
                    if cont > len(self.cidades):
                        ...
                        continue
                    sorteados.append(b)
                    c = (self.adjacencia(d[len(d) - 1], b) and d.append(
                        b) or False)  # VERIFICA SE CIDADES SAO ADJACENTES (EXISTE ROTA ENTRE ELAS)
                if len(
                        self.populacao) == self.tamPopulacao:  # COMO AS THREADS PODEM SE DIVIDIR EM MAIS ELEMENTOS QUE O TAMANHO ORIGINAL PRAMETRIZADO
                    return False  # DEVIDO AO ARREDONDAMENTO DE VEZES PARA CADA THREAD CRIAR ELEMENTOS
                res = self.geraFitnes(d,
                                      0)  # VERIFICO AQUI SE TAMANHO TOTAL DE ELEMENTOS JA FOI CRIADO ANTES SE INSERIR CADA ELEMENTO
                d.insert(0, res[0])
                d.insert(1, res[1])
    ...
    def adjacencia(self, a, b):
        return ((self.distancia[a][
                     b] < 9999 and a != b or False) and True or False)  # SE A DISTANCIA MENOR QUE 9999 AVALIA SE A!=B PARA EVITAR IR E VOLTAR NA MESMA CIDADE
    ...
    def geraFitnes(self, mat, pos):  # AVALIA CADA ROTA POSSIVEL DA POPULACAO
        try:
            ...
            mat = mat[pos:]
            f = self.cidades
            tenhoTodasCidades = ((self.__sublist(mat,
                                                 f) and True or False) and 1 or 100)  # VERIFICA SE ROTA SENDO AVALIADA POSSUI TODAS AS CIDADES
    ...
    def __crossover(self):  # IMENDA ROTAS MAIS BEM AVALIADAS EM ROTAS MAL AVALIADAS A PARTIR DE PONTO EM COMUM
        try:
            tamCrossOver = round(
                ceil(self.tamPopulacao / 2) * self.crossover)  # NUMERO DE INDIVIDUOS QUE SOFRERAO MUTACAO
            faixa = int(ceil(tamCrossOver / self.NUM_THREADS))
            r = int((
                                tamCrossOver > self.NUM_THREADS and self.NUM_THREADS or tamCrossOver))  # VERIFICA SE NUM THREADS E MAIOR QUE NUM ELEMENTOS QUE SOFRERAO MUTACAO
            threads = []
            for i in range(r):
                threads.append(self.__startThreads('self.geraCrossOver', {'ini': i + 1,
                                                                          'tam': faixa}))  # DISPARANDO THREADS PARA EFETUAR MUTACAO NOS PIORES ELEMENTOS DA POPULACAO EM FAIXAS
    ...
    def geraCrossOver(self, ini, tam, nodo=None):
        try:
            ...
            while ini < limite:
                ...
                v = {len(self.populacao[ini][:self.populacao[ini].index(nodo)]): 'esq', len(self.populacao[ini][
                                                                                            self.populacao[ini].index(
                                                                                                nodo):]) - 1: 'dir'}  # NUMERO DE ELEMENTOS A DIREITA E ESQUERD DA POSICAO DE CORTE
                g = {len(melhorRota[:melhorRota.index(nodo)]): 'esq', len(melhorRota[melhorRota.index(
                    nodo):]) - 1: 'dir'}  # NUMERO DE ELEMENTOS A DIREITA E ESQUERD DA POSICAO DE CORTE DA MELHOR ROTA
                ...
                if ladoV[1] == 'esq':
                    try:
                        if indiceNodo > 0:
                            del self.populacao[ini][
                                :indiceNodo - 1]  # DELETA ELEMENTOS A ESQUERDA DO INICIO ATE O INDICE DO VETOR DE JUNCAO
                        ...
                        for indice in range(indiceNodo):
                            ...
                            self.populacao[ini].insert(indice, melhorRota[
                                indiceNodoG])  # ADICIONANDO ELEMENTOS A DIREITA NO VETOR DE MELHOR ROTA
                    except Exception as e:
                        pass
                else:
                    try:
                ...
                c = 0
                try:
                    while not self.__sublist(self.populacao[ini],
                                             self.cidades) and c < 50:  # TENTO 20 VEZES INSERIR ELEMENTO NA LISTA PARA COMPLETA-LA
                        cont = 0
                        c += 1
                        ...
                        while elem in list and cont < 50:
                            ...
                            cont += 1
                        if cont < 50:
                            if self.adjacencia(elem, self.populacao[ini][len(self.populacao[
                                                                                 ini]) - 1]):  # VERIFICA ADJACENCIA ENTRE O ELEMENTO E O ULTIMO DO VETOR
                                self.populacao[ini].append(elem)
                                break
                except Exception as e:
                    print('Erro ao completar rota, ', e)
                res = self.geraFitnes(self.populacao[ini],
                                      0)  # VERIFICO AQUI SE TAMANHO TOTAL DE ELEMENTOS JA FOI CRIADO ANTES SE INSERIR CADA ELEMENTO
